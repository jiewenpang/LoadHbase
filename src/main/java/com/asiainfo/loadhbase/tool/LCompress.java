...
public class LCompress {
	...
	public static byte[] deCompress(byte[] ctx) {
		Input_Buffer ib = new Input_Buffer(ctx.length, ctx);
		Output_Buffer ob = new Output_Buffer();
		Decompressor de = new Decompressor(ib, ob);
		de.decompress();
		byte[] result = Bytes.copy(ob.getOutBuff().toByteArray(), 0, de.Output.count());
		return result;
	}
...
final class Compress {
	...
	final static byte lmask[] = { (byte) 0xff, (byte) 0xfe, (byte) 0xfc, (byte) 0xf8, (byte) 0xf0, (byte) 0xe0,
			(byte) 0xc0, (byte) 0x80, (byte) 0x00 };
	final static byte rmask[] = { (byte) 0x00, (byte) 0x01, (byte) 0x03, (byte) 0x07, (byte) 0x0f, (byte) 0x1f,
	...
	public static int spec_select_action(byte[] from_buf, int from_count, int action, byte[] to_buf) {
		...
		Output_Buffer out = new Output_Buffer();
...
final class Output_Buffer {
	...
	public ByteArrayOutputStream getOutBuff() {
		return OutBuff;
	}
...
final class Compressor extends Comp_Base {
	...
	private void output(int code) {
		...
		if (code >= 0) {
			 ...
			 * on the left.
			 */
			buf[bp] = (byte) ((buf[bp] & Compress.rmask[r_off]) | (code << r_off) & Compress.lmask[r_off]);
			bp++;
			bits -= (8 - r_off);
			code >>= 8 - r_off;
...
final class Decompressor extends Comp_Base {
	...
	public Decompressor(Input_Buffer in, Output_Buffer out) {
		...
		maxmaxcode = 1 << maxbits;
		if (maxbits > Compress.BITS) {
			System.err.println("stdin: compressed with " + maxbits + " bits, can only handle " + Compress.BITS
					+ " bits");
			// System.exit(1);
		}
		n_bits = Compress.INIT_BITS;
